...
class Sphere {
    ...
    void push_point(int i,int j) {
        vert[p++] = r * cos(i * M_PI / stacks) * cos(j * M_PI / slices);
        vert[p++] = r * cos(i * M_PI / stacks) * sin(j * M_PI / slices);
        vert[p++] = r * sin(i * M_PI / stacks);
    }
...
int main() {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        for(int i = 0;i < 10;i++) {
            ...
            // model = glm::rotate(model, glm::radians((float)glfwGetTime() * 100.0f + 20.0f * i), glm::vec3(0.0f, 1.0f, 0.0f));

            model = glm::translate(model, glm::vec3(
                i * 3.0f * sin((float)glfwGetTime() + 20.0f * i) ,
                0,
                i * 3.0f * cos((float)glfwGetTime() + 20.0f * i) 
            ));
            
            // rotate 是按照局部坐标系来的
            model = glm::rotate(model, glm::radians((float)glfwGetTime() * 100.0f + 20.0f * i), glm::vec3(1.0f + i, 0.3f + i * 0.1f, 0.5f));
